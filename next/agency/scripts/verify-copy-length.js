...
function calculatePercentageChange(original, refined) {
  if (original === 0) return refined > 0 ? 100 : 0;
  return ((refined - original) / original) * 100;
}

function verifyLengthConstraint(section, field, originalText, refinedText) {
  ...
  // Special handling for very short text (1-2 words like titles)
  // Allow up to 1 word increase for titles, otherwise use 10% rule
  let passes;
  if (originalWordCount <= 2) {
    // For titles, allow up to 1 word increase
    passes = refinedWordCount <= originalWordCount + 1;
  } else {
    // For longer text, use the 10% rule
    passes = percentageChange <= 10;
  }
