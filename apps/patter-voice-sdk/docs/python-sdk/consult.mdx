...
| Field | Default | Notes |
|---|---|---|
| `url` | `None` | Generic webhook endpoint. SSRF-validated at call start. **Mutually exclusive** with `openai_compatible` — set exactly one. |
| `openai_compatible` | `None` | Native OpenAI-compatible target (e.g. an OpenClaw agent) — no hand-written adapter. Use `ConsultConfig.openclaw(...)`. See [OpenClaw](#openclaw-native-target-no-adapter). |
...
| `reassurance` | `None` | Filler the agent speaks while the consult runs (Realtime mode only). The `openclaw` preset sets a sensible default. |
...
adapter entirely: `ConsultConfig.openclaw(...)` speaks OpenClaw's
...
| `base_url` | `http://127.0.0.1:18789/v1` | OpenClaw gateway (OpenAI-compatible base). |
| `api_key` | env `OPENCLAW_API_KEY` | Operator-grade bearer. Prefer the env var. |
| `timeout_s` | `30.0` | Phone-safe. Don't raise above 30 s on a live call. |
