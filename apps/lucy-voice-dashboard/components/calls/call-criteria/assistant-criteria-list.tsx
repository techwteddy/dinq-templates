...
import { Plus, GripVertical, Trash2, Pencil, Building2 } from 'lucide-react'
...
interface AssistantCriteriaListProps {
  organizationId: string
  assistantId: string
  onSummaryChange?: (summary: string) => void
}

export function AssistantCriteriaList({ organizationId, assistantId, onSummaryChange }: AssistantCriteriaListProps) {
  const t = useTranslations('settings.criteria')
  const tAssistant = useTranslations('assistants.criteria')
  const tCommon = useTranslations('common')
  const [orgCriteria, setOrgCriteria] = useState<CallCriterion[]>([])
  const [assistantCriteria, setAssistantCriteria] = useState<CallCriterion[]>([])
  ...
  async function loadCriteria() {
    ...
    // Load both org-level and assistant-specific criteria
    const [orgResult, assistantResult] = await Promise.all([
      getOrgLevelCriteria(organizationId),
      getAssistantCriteria(assistantId),
    ])

    if (orgResult.error) {
      toast.error(orgResult.error)
    } else {
      setOrgCriteria(orgResult.criteria)
    }

    if (assistantResult.error) {
      toast.error(assistantResult.error)
    } else {
      setAssistantCriteria(assistantResult.criteria)
    }
  ...
  useEffect(() => {
    ...
    const total = orgCriteria.length + assistantCriteria.length
    if (total === 0) {
      onSummaryChange('—')
    } else {
      const parts: string[] = []
      if (orgCriteria.length > 0) parts.push(`${orgCriteria.length} Org`)
      if (assistantCriteria.length > 0) parts.push(`${assistantCriteria.length} Assistent`)
      onSummaryChange(parts.join(', '))
    }
  }, [orgCriteria, assistantCriteria, onSummaryChange])
    ...
    startTransition(async () => {
      if (editingCriterion) {
        ...
        if (error) {
          ...
          setAssistantCriteria(prev => prev.map(c => c.id === criterion.id ? criterion : c))
          toast.success(t('criterionUpdated'))
          setIsDialogOpen(false)
        }
      } else {
        // Create new assistant-specific criterion
        ...
        if (error) {
          ...
          setAssistantCriteria(prev => [...prev, criterion])
  ...
  async function handleDelete() {
    ...
    startTransition(async () => {
      ...
      if (error) {
        ...
        setAssistantCriteria(prev => prev.filter(c => c.id !== deletingCriterion.id))
  ...
  async function handleToggleActive(criterion: CallCriterion) {
    startTransition(async () => {
      ...
      if (error) {
        ...
        setAssistantCriteria(prev => prev.map(c => c.id === updated.id ? updated : c))
      }
    })
  }
  ...
  async function handleDrop(e: React.DragEvent, targetCriterion: CallCriterion) {
    ...
    const newCriteria = [...assistantCriteria]
    ...
    setAssistantCriteria(newCriteria)
    setDraggedItem(null)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Assistant-Specific Criteria */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-medium">
            {tAssistant('assistantCriteria')}
          </h4>
        ...
        {assistantCriteria.length === 0 ? (
          <div className="text-center py-6 text-muted-foreground border-2 border-dashed rounded-lg">
            <p className="text-sm">{tAssistant('noAssistantCriteria')}</p>
            <Button variant="link" onClick={openCreateDialog} className="mt-1 text-sm">
              {tAssistant('addFirstAssistantCriterion')}
            </Button>
          </div>
        ) : (
          <div className="space-y-2">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingCriterion ? t('editCriterionDescription') : tAssistant('createAssistantCriterionDescription')}
            </DialogDescription>
          </DialogHeader>
